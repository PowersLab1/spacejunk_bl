...
export const globalState = new function () {
	...
	this.gameTutorial = [
		"To fix your rocket and escape, you need to catch these spare parts. /n But here's the catch: you can't see the spaceships because they are too /n high above, so you will have to guess where the spare parts will drop.",
		...
		"Let’s practice using your beam. /n Press the UP (↑) key to turn on the beam. /n To widen and narrow it press the UP (↑) and DOWN (↓) keys /n To move the space rover around press the LEFT (←) and RIGHT (→) keys",
	];
	this.trialIntros = [
	...
	this.difficultyWarning = ["Nice work! The game has 4 levels. /n In later levels there will be mutiple spaceships that drop off spare parts. /n  One spaceship will finish dropping its spare parts before the next one starts. As the spaceships are too high above to see, you will need to guess when a new one arrives.",
	                       "Hint: new spaceships will often drop parts in new locations, and bigger spaceships will drop parts over a wider area."];
