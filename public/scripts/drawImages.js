...
// import backgroundLevel1 from "../assets/background_level1.jpg"
...
/**
 * Displays a warning when a user tries to complete a trial when the beam is inactive
 */
export function drawBeamInactiveWarning(globalState, ctx) {
    const fillCopy = ctx.fillStyle;
    ctx.fillStyle = "red";
  	ctx.font = `${scaleText(20, globalState)}px ${globalState.textFont}`;
	ctx.fillText("Beam not Activated. Press UP (↑) to activate", (globalState.canvasWidth/2), (globalState.canvasHeight/10)*3);
	ctx.fillStyle = fillCopy;
};
...
export function drawTrialHint(globalState, ctx) {
    ...
    if (globalState.level === 1) {
        ctx.fillStyle = "green";
        ctx.fillText(`This is where the last spare part fell`, globalState.canvasWidth/2, (globalState.canvasHeight/10)*1);
        ctx.fillStyle = "white";
...
export function drawBeam(globalState, ctx) {
	...
	if (globalState.beamNeedsActivation && !globalState.playerSettings.beamActive) {
	    ctx.fillStyle = globalState.playerSettings.beamColorDeactivated;
	}
	else {
	    ctx.fillStyle = globalState.playerSettings.beamColorActive;
	}
	ctx.closePath();
	ctx.fill();
	ctx.globalAlpha = 1.0;
...
export function drawPower(globalState, ctx) {
	...
	ctx.fillStyle = "red";
	ctx.textAlign = "center";
	ctx.font = `${scaleText(15, globalState)}px ${globalState.textFont}`;
...
export function drawCountdown(globalState, ctx) {
    ...
    ctx.fillStyle = "white";
    ctx.fillText(`Move rover and adjust power to catch spare parts`, globalState.canvasWidth/2, (globalState.canvasHeight/10)*1);
}
...
export function renderBackground (globalState, ctx) {
    ...
    // if (isNaN(globalState.level)) {globalState.level =1 }
...
export function drawSimpleResult(globalState, ctx) {
	...
	if (globalState.trialTimeLeft <= 0) {
	    if (globalState.repeatTrialOnTimeout) {
		    result = "Timed Out! Repeating Round!"
	    }
	    else {
	        result = "Timed Out!"
	    }
...
/**
 * Draws the Total Score the User has at the end of the round
 * 
 * @param {*} globalState The global state of the game
 * @param {*} ctx The canvas object of the game
 */
...
/**
 * Draws the Total Rewards the User has at the end of the round
 * 
 * @param {*} globalState The global state of the game
 * @param {*} ctx The canvas object of the game
 */
export function drawTotalRewards(globalState, ctx) {
	// Display the text
	ctx.fillStyle = "green";
	ctx.textAlign = "center";
	ctx.font = `${scaleText(20, globalState)}px ${globalState.textFont}`;
	ctx.fillText(`${globalState.rewardName}: ${Math.floor(globalState.totalScore/globalState.rewardDivision)}`, (globalState.canvasWidth/4)*1, (globalState.canvasHeight/10)*2);
}


/**
 * Draws the Coins / Rewards the User has
 * @param {*} globalState The global state of the game
 * @param {*} ctx The canvas object of the game
 */
export function displayTotalRewards(globalState, ctx) {
	// Display the text
	ctx.fillStyle = "green";
	ctx.textAlign = "center";
	ctx.font = `${scaleText(20, globalState)}px ${globalState.textFont}`;
	ctx.fillText(`${globalState.rewardName}: ${Math.floor(globalState.totalScore/globalState.rewardDivision)}`, (globalState.canvasWidth/10)*1.5, (globalState.canvasHeight/10));
}
...
export function displayCurrentLevel(globalState, ctx) {
	ctx.fillStyle = "green";
	ctx.textAlign = "center";
	ctx.font = `${scaleText(20, globalState)}px ${globalState.textFont}`;
	const level = globalState.level - 1;
	ctx.fillText(`Level: ${level}`, (globalState.canvasWidth / 10) * 8.5, (globalState.canvasHeight / 7.5));
}

export function drawSpaceShips(globalState, ctx) {
	
	let w = globalState.canvasWidth * 0.62;
	let y = globalState.canvasHeight * 0.21;
	let x = globalState.canvasWidth * 0.5 - w / 2;
	let h = globalState.canvasHeight * 0.6;

	ctx.drawImage(
		document.getElementById("spaceShips"),
		x, 
		y,
		w, 
		h
