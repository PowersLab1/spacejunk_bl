 ...
 */
export function handleMovement(globalState, allowHorizontal = true) {
    // If we are allowed to move the rover
    if (allowHorizontal) {
...
export const movers = {
	...
	moveUp: (playerSettings) => {
	    if (!playerSettings.beamActive) {
    	    playerSettings.beamActive = true;
    	    playerSettings.inactivePress = false;
	    }
	    else {
